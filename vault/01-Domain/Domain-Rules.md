...
2. **contributionRates**: This table defines historical rates (amt) related to a type of contribution (contributionHeadsId), a reference to how it was decided (reference to minutes of a meeting) , and Period (fromDt to toDt). A null in toDt means the current applicable rate.
	...
	- *rate-period coverage policy (Track-A 1.9)*: The applicable rate is resolved at `transactionDateTime`, not at the period start. If a payment is captured in April for a January period, the rate effective in April applies — even if that rate only became effective in February. This is **accepted policy**. When the resolved rate's `fromDt` is later than the earliest selected period's start, the API returns a non-blocking `warning` in the response and the UI shows an amber notice. The contribution is posted normally.
