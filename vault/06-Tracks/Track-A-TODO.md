...
| 1.10 | Contribution period predates current owner's ownership start | ⬜      | **Gap found 2026-09-22.** A unit's current owner (e.g. ownership `fromDt` = 1 Sep 2026) can currently post/pay monthly contributions for periods before their ownership started (e.g. Jan–Mar 2026), because Contribution Period Constraints only restrict to "current year" (no check against the paying owner's or unit's active-ownership start date). Unclear whether this is a real bug or acceptable: dues are arguably owed by the *unit*, not the individual, and clearing pre-purchase arrears at resale time is common real-world practice — but nothing today distinguishes "intentional arrears settlement" from "accidental wrong-period selection." **Recommended resolution (mirrors 1.9's precedent):** do not block; add a non-blocking warning (same pattern as `checkRatePeriodCoverage`) when a selected period's start predates the currently active owner's `fromDt` on the unit, e.g. "Selected period predates current ownership — confirm this is an intentional arrears payment." Needs a product decision before implementing; not yet built. |
